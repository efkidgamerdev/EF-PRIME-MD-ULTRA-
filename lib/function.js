...
// baileys not needed in function.js
...
const getGroupAdmins = (participants) => {
    // ✅ FIX v7: participant.id is a LID in v7 — include both id and lid so callers can match either
    let admins = []
    for (let i of participants) {
        if (i.admin === 'superadmin' || i.admin === 'admin') {
            admins.push(i.id)
            if (i.lid && i.lid !== i.id) admins.push(i.lid)
        }
    }
    return admins
}
